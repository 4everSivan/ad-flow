...
4. **统一施工蓝图与物理运行现实（Deployment Reality）**：
   部署指南（`docs/guide/`）是设计蓝图，实测报告（`local/deploy_report.md`）是物理运行现实。所有后续自动化测试、联调与重启必须以实机报告为唯一事实依据，同时将所有运行时数据严格隔离在 `local/` 目录中。
...
### 4.3 初始化产物目录骨架全景 (22 个标准化文件)

执行 `$ad-flow` 初始化后，目标工程将严密生成以下 22 个治理文件及运行时隔离区：

```text
<目标工程根目录>/
├── AGENTS.md                         # 【1/22 项目协作宪法】含 <!-- @ad-flow: initialized --> 标签与 8 大规范
├── CHANGELOG.md                      # 【2/22 版本更新日志】SemVer 规范，[Unreleased] 挂接变更与任务卡
├── adflow.config.json                # 【3/22 治理配置文件】定义工程元数据与流转配置
│
├── local/                            # 【本地隔离运行区】（已加入 .gitignore，AI 严禁删除）
│   ├── deploy_report.md              # 【实机部署报告】记录实际端口、DB路径、PID、端点（测试与重部署唯一依据）
│   ├── data/                         # 本地数据库与持久化数据存储目录
│   └── logs/                         # 本地运行时与调试日志输出目录
│
├── docs/                             # 【工程文档中心】
│   ├── README.md                     # 【4/22 文档中心总览】统一四章结构（简介/索引/规范/发版）+ 英文元数据
│   ├── index.json                    # 【5/22 顶层机器路由】全景目录路由中枢
│   │
│   ├── assets/                       # 【文档与工程静态资源库】统一收拢架构图、App 截图、视觉素材
│   │   └── README.md                 # 【6/22 资源库索引与规范】命名契约、体积限制与引用格式
│   │
│   ├── guide/                        # 【操作指引区】标准操作手册（部署、运维、联调）
│   │   ├── README.md                 # 【7/22 指南目录索引】指南编写四要素与实机部署事实依据原则
│   │   ├── index.json                # 【8/22 指南机器索引】操作指南结构化清单与产物映射
│   │   └── 01-本地部署指南.md        # 【9/22 本地部署指南】标准本地环境搭建与服务启动手册（施工蓝图）
│   │
│   ├── devel/                        # 【开发核心区】研发内场作战室
│   │   ├── README.md                 # 【10/22 开发者主索引】内场四层架构与双轨事项流转总线
│   │   ├── index.json                # 【11/22 内场机器路由】研发内场子系统总账中枢
│   │   │
│   │   ├── design/                   # 【现行设计基线】系统唯一真理（@topic 锚点）
│   │   │   └── README.md             # 【12/22 设计基线大纲】内嵌 8 节标准设计方案大纲（严禁伪造虚假设计）
│   │   │
│   │   ├── change/                   # 【变更核验卡池】日常 Bug 修复与微调
│   │   │   ├── README.md             # 【13/22 变更核验指南】抗漂移原理、前后对比表要求与核验代签
│   │   │   ├── index.json            # 【14/22 变更总账中枢】Topic 映射、状态追踪与分支路由
│   │   │   └── template.json         # 【15/22 变更卡模板】标准 C 卡模板（含 branch 分支标签）
│   │   │
│   │   ├── task/                     # 【阶段研发任务】Phase/Milestone 交付流
│   │   │   ├── README.md             # 【16/22 任务研发指南】DAG 拓扑算法、能力交付与完成定义 (DoD)
│   │   │   ├── index.json            # 【17/22 任务总账中枢】Milestone 汇聚、拓扑依赖与分支路由
│   │   │   └── template.json         # 【18/22 任务卡模板】标准 T 卡模板（含 branch 分支标签）
│   │   │
│   │   └── todo/                     # 【待办缓冲池】零沉淀事项池
│   │       ├── README.md             # 【19/22 待办缓冲指南】落地建卡即物理删除的零沉淀纪律
│   │       ├── now.md                # 【20/22 缺陷待办缓冲】当前缺陷、回调与微调暂存表
│   │       └── future.md             # 【21/22 特性待办缓冲】远期特性与技术债暂存表
│   │
│   └── archive/                      # 【历史归档区】版本发版打 Tag 后整体封存（只读冻结）
│       └── README.md                 # 【22/22 历史归档索引】发版封箱四步 SOP 与只读规则
│
└── _adflow_backup/                   # 【安全备份区】仅进行中项目接入时生成（AI 严禁删除，仅人类核验后清理）
    ├── README.md                     # 备份安全声明
    └── original_docs/                # 原有文档完整备份（递归归拢 docs/、openspec/、superpower/、specs/ 及根目录 md）
```
...
## 5. 本地部署与环境隔离规约 (Deployment & Local Isolation)

`ad-flow` 深度贯彻 **“施工蓝图与物理运行实况解耦”** 的部署哲学：
...
### 3. 环境隔离与绝对安全保护
- 部署与测试运行时产生的所有临时文件、数据库（SQLite / DuckDB 等）、缓存及日志，**一律强制收拢于 `local/` 目录**，严禁向源码树扩散；
- **【严格红线】`local/` 仅人类手工清理**：**AI Agent 严禁擅自删除或重置 `local/` 目录**，运行实况与调试数据的生命周期 100% 由人类在宿主机终端手动维护。
