...
│   │       └── README.md             # 【23/23 历史归档索引】发版封箱四步 SOP 与只读规则
│   │
│   └── archive/                      # 【历史归档区】同上
...
- **流水线步骤 (Pipeline)**：
  - **初次初始化流**：`Step 1: 资产勘察与版本检测` $\rightarrow$ `Step 2: 人工确认守卫` $\rightarrow$ `Step 3: docs/ 原子移走与安全快照` $\rightarrow$ `Step 4: AGENTS.md 宪法融合 (v1.1.0)` $\rightarrow$ `Step 5: 10大目录独立 README 矩阵` $\rightarrow$ `Step 6: 中枢总账与模板注入` $\rightarrow$ **`Step 7: 旧文档消化与设计基线重构 (Legacy Synthesis)`** $\rightarrow$ `Step 8: DoD 完整性硬断言与交付汇报`。
